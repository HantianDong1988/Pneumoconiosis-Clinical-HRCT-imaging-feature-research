...
def predict(image_path, char):
    ...
    if char == 'A':
        y_label = np.append(y_label, [[1, 0, 0, 0]], axis=0)
    if char == 'B':
        y_label = np.append(y_label, [[0, 1, 0, 0]], axis=0)
    if char == 'C':
        y_label = np.append(y_label, [[0, 0, 1, 0]], axis=0)
    if char == 'D':
        y_label = np.append(y_label, [[0, 0, 0, 1]], axis=0)
    # if predicted.item() == 0:
...
for i in range(65, 69):
    ...
    for f in os.listdir("ct2_dataset/x-new/" + chr(i)):
        ...
        predict("ct2_dataset/x-new/" + chr(i) + "/" + str(f), chr(i))
        # 获取图像类标即为文件夹名称
        Y.append(chr(i))
print(y_label.shape)
print(y_score.shape)
...
plt.savefig('0824/ROC.pdf', format='pdf')
plt.show()

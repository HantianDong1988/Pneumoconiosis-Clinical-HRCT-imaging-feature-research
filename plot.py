...
densnet_att = pd.read_csv('csv/densnet_att.csv', index_col=None)
...
densnet_att = densnet_att.values[:, 1:3]
...
for i in range(30):
    if i % 4 == 0:
        ...
        densnet_att_list.append(densnet_att[i, :])
        mobi.append(MobileNet[i, :])
dens = np.array(dens)
res = np.array(res)
google = np.array(google)
shuff = np.array(shuff)
densnet_att = np.array(densnet_att_list)
mobi = np.array(mobi)
myparams = {
...
fig1 = plt.figure(1)
plt.grid()
plt.plot(x, dens[:, 0], linewidth=2, label='DenseNet121', marker='o',
         markersize=6)
...
plt.plot(x, shuff[:, 0], linewidth=2, label='ShuffleNet', marker='v',
         markersize=6)
plt.plot(x, densnet_att[:, 0], linewidth=2, label='DenseNet-Att', color='r', marker='*',
         markersize=6)
axes1 = plt.gca()
...
plt.ylabel('Acc')
plt.xlabel('Epoch')
plt.savefig("0824/Acc.pdf", dpi=72, format='pdf')
plt.show()
